...
public class PacketManager {
    ...
    private static final long DEAULT_WAIT_AUTH_TIME_OUT = 5 * 1000;
    ...
    private long waitAuthTimeout = DEAULT_WAIT_AUTH_TIME_OUT;
    ...
    public boolean isAutoAuth() {
        return socketConnection.isAutoAuth;
    }
    ...
    private void startPacketTask(Packet packet) {
        startPacketTask(packet, sendTimeout);
    }

    private void startPacketTask(Packet packet, long timeout) {
    ...
    boolean handleWaitAuth(Packet packet) {
        return socketConnection.handleWaitAuth(packet);
    }
    ...
    void handleFailedPacket(Packet packet) {
        ...
        if (packet.isShouldWaitAuth() && !isAuthed()) {
            ...
            if (!packet.isExpired() && packet.getReSendTimes() < resendTimes && !packet.isHeartBeatPacket()) {
